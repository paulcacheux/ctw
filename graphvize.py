...
#        return name
    ...
    def draw_node_pm(self, node):
        name = self.next_name()
        self.s += '\t{} [label="pe={:.3f} | pm={:.3f} | as={}"];\n'.format(name, float(node.pe), float(node.pm), node.count)
        for c in node.children:
            if c is not None:
                sub_name = self.draw_node_pm(c)
                self.s += "\t{} -> {} [label={}];\n".format(name, sub_name, c.value)
        return name
